...
function generateRandomParticleInSphere(radius) {
    ...
    return [x, y, z];
          ...
          <li>By slowly rotating the particles generated within the sphere around the x-axis, the particles will appear to swirl gently, creating an illusion of a snowstorm. We can modify the rotation of the particles using <a href="https://sbcode.net/react-three-fiber/use-frame/">React Three Fiber's useFrame hook</a> to execute code before every rendered frame.</li>
          <pre><code> {`
useFrame((state, delta) => {
    snow.current.rotation.x -= delta * 0.05
      ...
      Simple geometry (i.e. sphere, cone) meshes stacked and scaled to build a snowman in a component. The color of the snowman can be configured by a React prop called "evil" which describes whether the snowman should be green or red. Red snowmen are evil. Green snowmen are good.
        <pre><code> {`
