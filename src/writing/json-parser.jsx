      ...
      <div id="blog" className="blog">
        <div id="writing" className="writing">
          ...
          <p>
            ...
            it's a configuration file, data exchange between APIs, or even
            simple application state management, we all interact with JSON
            daily. But have you ever wondered how JSON parsing actually works
            behind the scenes?
          </p>
          <h2>The Goal</h2>
          <p>
            The goal of this project was simple: Create a basic JSON validator
            that can parse JSON strings and check their validity. This isn't
            meant to be a production-ready tool, but rather a fun exercise in
          ...
          <p>
            JSON.parse() is the go-to tool for parsing JSON strings in
            JavaScript. It’s easy, fast, and works for almost all scenarios. But
            I wanted to go a step further and understand how a JSON parser
          ...
          <p>
            ...
            and more manageable. These simpler rules do not require recursion,
            so we can implement basic parsing functions for them without
            worrying about nesting or complex structures. Once these fundamental
            cases are handled, we can gradually work up to more complex,
            recursive grammars such as arrays and JSON objects.{' '}
          </p>{' '}
          <p>
            {' '}
            ...
            tackling more challenging structures. By parsing these smaller
            grammar rules first, we create a solid foundation, which can then be
          ...
          <p>
            Recursive descent parsing is especially suitable for these
            structures because it allows us to keep track of the nesting levels.
            Each recursive call parses a new level of the structure, whether
            it’s an array within an array or a JSON object containing another
            JSON object.
          </p>
          <p>
          ...
          <p>
            ...
            defined grammar rules. The process starts by checking each part of
            the string, such as strings, numbers, booleans, arrays, or objects,
            and recursively parsing nested structures. If the parsing succeeds,
            we continue validating the remaining part of the JSON string.
            However, if we encounter an unexpected token or cannot match the
            expected structure at any point, the parsing fails, and we
            immediately deem the JSON as invalid. This approach ensures that the
            entire JSON structure adheres to the formal grammar, and any
            deviation from it—such as mismatched brackets, incorrect types, or
            missing values—results in an invalid JSON string.
          </p>
          <h2>Challenges Faced</h2>
          <ul>
            <li>
              <strong>Handling escaped quotes in strings:</strong> One of the
              key challenges when parsing JSON is correctly handling escaped
              characters, such as <code>\"</code>, within strings. Since JSON
              strings are enclosed in double quotes, we must carefully track
              when a quote is part of the string (escaped) or when it marks the
              end of the string. Failing to correctly parse escaped quotes could
              lead to incorrect string values or prematurely terminating the
              string.
            </li>
            <li>
              <strong>
              ...
              whitespace, but it's crucial to correctly validate that
              separators, like colons (:) between keys and values or commas (,)
              between elements in arrays and objects, are correctly placed.
              Missing or misplaced separators could break the structure of the
              JSON and lead to invalid data. The parser needs to handle these
              edge cases by skipping over valid whitespace and ensuring the
              correct presence of colons and commas where needed.
            </li>
            <li>
              <strong>String formatting limitation:</strong> Currently, our
              parser requires all strings to be enclosed in double quotes, as
              per the grammar specification. This means that single-quoted
              strings are not supported, and any input using single quotes will
              result in a failure.
            </li>
          </ul>
